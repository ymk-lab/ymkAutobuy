...
**SevereTrim FastReentry**:
現行預設進攻規則：滿倉為常態；若（高波動且跌破 MA100）或（收盤連 N 日跌破 MA200）則降到 trim 權重（預設 50%）；站回 MA50 立刻回到滿倉。
_Avoid_: 動輒歸零、用過短均線日常進出

**HoldHigh DipScale**:
高位滿倉保留；僅在趨勢均線確認反轉後賣出；其後若相對大盤超跌則按步加倉，最多加到一半；站回趨勢均線再全倉。
_Avoid_: 因價格創新高而主動減倉、未確認反轉就反覆進出、超跌一次加滿
...
**Fast Re-Entry**:
減倉後，用比出場更快的條件回到滿倉（例如站回 MA50），避免踏空反彈。
