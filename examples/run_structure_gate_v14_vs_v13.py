#!/usr/bin/env python3
"""Structure Gate v14 vs v13 (SPY50/QQQ50) on 2025–2026 window.

v14 = v13 locks/defense + enter stock immediately when unlocked
      + minimum 3-day hold before soft exit (risk cash still pierces).

Usage::

    PYTHONPATH=src:examples python examples/run_structure_gate_v14_vs_v13.py
    PYTHONPATH=src:examples python examples/run_structure_gate_v14_vs_v13.py 2025-08-07 2026-08-13
"""
...
OUT = ROOT / "examples" / "data" / "structure_gate_v14_vs_v13"
...
def run_preset(
    ...
    out = {
        ...
        "weights": weights,
        "cfg": {
            "mode_enter_immediate": cfg.mode_enter_immediate,
            "mode_min_hold_days": cfg.mode_min_hold_days,
...
def main() -> int:
    ...
    v14 = run_preset("v14", StructureGateConfig.v14(), dict(V14_BOOK_WEIGHTS), frames, start, end)

    delta = {
        "ret_pp": (v14["blend"]["total_return"] - v13["blend"]["total_return"]) * 100,
        "maxdd_pp": (v14["blend"]["max_drawdown"] - v13["blend"]["max_drawdown"]) * 100,
        "trades": v14["blend"]["n_trades"] - v13["blend"]["n_trades"],
        "vs_spy_pp": v14["blend"]["vs_spy_bh_pp"] - v13["blend"]["vs_spy_bh_pp"],
    }
    report = {
        ...
        "v13": v13,
        "v14": v14,
        "delta_v14_minus_v13": delta,
        "note": (
            "v14: mode_enter_immediate + mode_min_hold_days=3; "
            "locks (sticky/thrust/…) unchanged from v13"
        ),
    }
    ...
    lines = [
        "=== Structure Gate v14 vs v13 ===",
        f"窗：{start.date()} → {end.date()}｜資本 ${CAPITAL:,.0f}｜權重 SPY50/QQQ50",
        "",
        f"v13: {v13['blend']['total_return']*100:+.2f}%｜maxDD {v13['blend']['max_drawdown']*100:.2f}%｜"
        f"trades {v13['blend']['n_trades']}｜vsSPY {v13['blend']['vs_spy_bh_pp']:+.1f}pp",
        f"v14: {v14['blend']['total_return']*100:+.2f}%｜maxDD {v14['blend']['max_drawdown']*100:.2f}%｜"
        f"trades {v14['blend']['n_trades']}｜vsSPY {v14['blend']['vs_spy_bh_pp']:+.1f}pp",
        f"Δ(v14−v13): ret {delta['ret_pp']:+.2f}pp｜maxDD {delta['maxdd_pp']:+.2f}pp｜trades {delta['trades']:+d}",
        "",
        "v14 規則：無鎖即進 ers/strong；進場後最少 hold 3 日（harsh/mild cash 仍可出場）；鎖與 v13 相同。",
    ]
    txt = "\n".join(lines) + "\n"
