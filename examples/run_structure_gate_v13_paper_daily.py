...
DEFAULT_OUT = ROOT / "examples" / "data" / "structure_gate_v13_paper"
...
def main() -> int:
    ...
    try:
        ...
        need_marks = sorted(set(positions) | set(target) | {f"{b}.US" for b in WEIGHTS})
        ...
        # v13 sleeves are ETF/bench — use bench_slippage_bps (default 3).
        ...
        if mode == "signal" or not submit:
            ...
            state_path.write_text(
                json.dumps({"asof": asof, "submitted": False, "at": str(ts), "preset": "v13"}, indent=2)
                + "\n"
            )
            return 0
