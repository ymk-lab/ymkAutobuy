#!/usr/bin/env python3
"""Structure Gate v15 vs v13 (SPY50/QQQ50) on 2025–2026 window.

v15 = v13 locks/defense + dual trail 20/50 (sticky/strong/ers_lag were 60).

Usage::

    PYTHONPATH=src:examples python examples/run_structure_gate_v15_vs_v13.py
    PYTHONPATH=src:examples python examples/run_structure_gate_v15_vs_v13.py 2025-08-07 2026-08-13
"""
...
OUT = ROOT / "examples" / "data" / "structure_gate_v15_vs_v13"
...
def main() -> int:
    ...
    v15 = run_preset("v15", StructureGateConfig.v15(), dict(V15_BOOK_WEIGHTS), frames, start, end)

    delta = {
        "ret_pp": (v15["blend"]["total_return"] - v13["blend"]["total_return"]) * 100,
        "maxdd_pp": (v15["blend"]["max_drawdown"] - v13["blend"]["max_drawdown"]) * 100,
        "trades": v15["blend"]["n_trades"] - v13["blend"]["n_trades"],
        "vs_spy_pp": v15["blend"]["vs_spy_bh_pp"] - v13["blend"]["vs_spy_bh_pp"],
    }
    report = {
        ...
        "note": (
            "v15: sticky_trail/strong/ers_lag lookback 60→50 (trail 20/50); "
    ...
    lines = [
        ...
        f"v15 (20/50): {v15['blend']['total_return']*100:+.2f}%｜maxDD {v15['blend']['max_drawdown']*100:.2f}%｜"
        f"trades {v15['blend']['n_trades']}｜vsSPY {v15['blend']['vs_spy_bh_pp']:+.1f}pp",
        f"Δ(v15−v13): ret {delta['ret_pp']:+.2f}pp｜maxDD {delta['maxdd_pp']:+.2f}pp｜trades {delta['trades']:+d}",
        "",
        "v15 規則：雙窗 trail 20/50（sticky/strong/ers_lag 長窗 60→50）；模式滯後與防守同 v13。",
    ]
    txt = "\n".join(lines) + "\n"
